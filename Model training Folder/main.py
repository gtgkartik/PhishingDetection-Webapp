...
preprocessed_features = extract_features(url)
print(preprocessed_features)

# Loading the trained model from the internet
loaded_model = pickle.load(open('./model_phishing_webpage_classifer94.plk' , 'rb'))
prediction = loaded_model.predict(preprocessed_features)

def final(num):
    if num == 1:
        print("Phishing Link")
    else :
        print("Not Phishing Link")
